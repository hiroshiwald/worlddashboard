...
// Brief, Developments, and Entities are DB-backed and render independently
// of the live feed items array (see TabContent.tsx) — every other tab needs
// items to render at all, so only they should show the live-feed
// loading/empty states.
function isItemsDependentTab(activeTab: TabKey): boolean {
  return activeTab !== "brief" && activeTab !== "developments" && activeTab !== "entities";
}
    ...
    <div className={`h-screen flex flex-col ${t.bg} transition-colors duration-200 ${dark ? "dark-scrollbar" : ""}`}>
      ...
      <div className={`flex-1 overflow-auto min-h-0 ${dark ? "dark-scrollbar" : ""}`}>
        ...
        <TabContent
          ...
          onEntityClick={handleEntityClick} onEntitySelect={setPanelEntityId} onCandidatesChanged={handleCandidatesChanged}
        />
