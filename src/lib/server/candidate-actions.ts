...
import { checkAndWriteEntityFame } from "./fame-sweep";

// Accept/dismiss candidate logic shared by /api/candidates POST and
// /api/candidates/bulk POST. Lives here rather than being exported directly
// from src/app/api/candidates/route.ts because Next.js's App Router only
// permits a route.ts file to export recognized HTTP handlers and route
// segment config — any other export fails `next build`'s route type check.
...
export function isBoundedNonEmpty(value: unknown): value is string {
...
export async function loadCandidate(sql: Sql, nameNorm: string): Promise<CandidateSnapshot | null> {
...
export async function deleteCandidate(sql: Sql, nameNorm: string): Promise<void> {
...
export async function acceptCandidate(sql: Sql, candidate: CandidateSnapshot, type: string): Promise<AcceptedEntity | null> {
...
export async function inlineFameCheck(sql: Sql, entity: AcceptedEntity, canonicalName: string): Promise<void> {
  try {
...
export async function dismissCandidate(sql: Sql, candidate: CandidateSnapshot): Promise<boolean> {
  const result = await sql`
    INSERT INTO entities (canonical_name, type, status, first_seen_at, last_seen_at)
...
export const CONFLICT_MESSAGE = "Could not resolve a naming conflict for this candidate — please retry";
